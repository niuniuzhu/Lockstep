Shader "Game/Bump_SpecularLight" {
	Properties {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 fragmentShader(v2f i) : SV_Target
            {
				...
				float3 normalDir = normalize(UnpackNormal(normalMap));//不用转换到切线空间?

                fixed4 diff = saturate(dot(i.lightDir, normalDir));
  
                fixed nh = saturate(dot(normalDir, normalize(i.viewDir + i.lightDir)));  
